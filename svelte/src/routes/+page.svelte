...
<div
	class="font-inter grid min-h-screen grid-rows-[auto_1fr_auto] bg-linear-to-br from-amber-50 via-orange-50 to-yellow-50"
>
	<!-- Header -->
	<header class="bg-linear-to-r from-amber-900 via-orange-900 to-yellow-900 text-white shadow-xl">
		...
		<div class="mx-auto max-w-4xl px-4 py-8 sm:px-6 sm:py-12 lg:px-8">
				...
				<div class="mx-auto max-w-md">
					...
					<div class="flex gap-3">
						...
						<button
							...
							class="transform rounded-xl bg-linear-to-r from-amber-600 to-orange-600 px-6 py-3 font-medium text-white transition-all duration-200 hover:scale-105 hover:from-amber-700 hover:to-orange-700 focus:ring-2 focus:ring-amber-500 focus:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50"
						>
							{#if isLoading}
			...
			<div class="mb-8 text-center">
				...
				<p class="text-xs text-amber-500">
					API: GET localhost:8080/api/delivery/status/$trackingId
				</p>
			</div>
			...
			{#if deliveryStatus}
				<div class="rounded-3xl border-2 border-amber-100 bg-white p-6 shadow-2xl sm:p-8">
					...
					{#if deliveryStatus.status !== 'not-found' && deliveryStatus.status !== 'error'}
						<div class="rounded-2xl bg-linear-to-r from-amber-50 to-orange-50 p-6">
				...
				<div class="mt-8 rounded-3xl border-2 border-amber-100 bg-white p-6 shadow-2xl sm:p-8">
					...
					{#if isLoadingReceipt}
						<div class="flex items-center justify-center py-8">
							<div class="flex items-center space-x-3">
								<div
									class="h-6 w-6 animate-spin rounded-full border-2 border-amber-600 border-t-transparent"
								></div>
								<span class="text-amber-700">Loading receipt...</span>
							</div>
						</div>
					{:else if deliveryReceipt}
						<div class="rounded-2xl bg-linear-to-br from-amber-50 to-orange-50 p-6">
							<div class="mb-4 border-b border-amber-200 pb-4">
								<h4 class="font-semibold text-amber-900">Receipt Details</h4>
								<p class="text-sm text-amber-600">
									Tracking ID: <span class="font-mono font-bold">{deliveryReceipt.id}</span>
								</p>
							</div>
							...
							<div class="space-y-3">
								<div class="flex items-center justify-between py-2">
									...
									<span class="font-mono text-amber-900"
										>¥{deliveryReceipt.deliveryFee.toFixed(2)}</span
									>
								</div>
								<div class="flex items-center justify-between py-2">
								...
								<div class="border-t border-amber-300 pt-3">
									<div class="flex items-center justify-between py-2">
										...
										<span class="font-mono text-lg font-bold text-amber-900"
											>¥{deliveryReceipt.total.toFixed(2)}</span
										>
									</div>
								</div>
							</div>
					...
					{:else}
						<div class="rounded-2xl bg-amber-50 p-6 text-center">
							...
							<p class="mt-1 text-sm text-amber-600">
								Receipt may be generated after delivery completion
							</p>
						</div>
					{/if}
				</div>
	...
	<!-- Footer -->
	<footer class="bg-linear-to-r from-amber-900 via-orange-900 to-yellow-900 text-white">
